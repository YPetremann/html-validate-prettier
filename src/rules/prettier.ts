  ...
  public setup(): void {
    const messages = {
      ...
      replace: (deleteCode: string, insertCode: string) =>
        `Replace "${showInvisibles(deleteCode)}" with "${showInvisibles(insertCode)}"`,
    };
    ...
    this.on('dom:ready', (event: DOMReadyEvent) => {
      ...
      const prettierOptions = Object.assign({}, initialOptions, prettierRcOptions, { filepath: filename });
      ...
      differences.forEach(difference => {
        ...
        switch (difference.operation) {
          case INSERT:
            this.report(null, messages.insert(difference.insertText), location);
            break;
          case DELETE:
            this.report(null, messages.delete(difference.deleteText), location);
            break;
          case REPLACE:
            this.report(null, messages.replace(difference.deleteText, difference.insertText), location);
            break;
        }
      });
    });
